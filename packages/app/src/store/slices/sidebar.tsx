...
export const sidebarSlice = createSlice({
  ...
  initialState,
  reducers: {
    bindMenu: (state, { payload }: PayloadAction<{ key: '@@APP' | ModuleNames, menu?: SidebarMenu }>) => {
      if (payload.menu)
...
export const selectSidebarLinks = createSelector([selectMenus], (menus) => {
  return _(menus).values().map(m => m.links).flatten().value()
})

export const selectSidebarGroups = createSelector([selectMenus], (menus) => {
  return _(menus).values().map(m => m.groups).flatten().value()
})
